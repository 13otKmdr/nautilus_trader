...
class VWAPEngine(Indicator):
    ...
    def _should_reset(self, bar_dt: datetime) -> bool:
        ...
        return bool(current_date > self._last_reset_date and bar_dt.hour >= self._session_reset_hour)
    ...
    def get_band(self, level: int, upper: bool) -> float:
        ...
        bands = {
            1: (self._current_state.sd1_upper, self._current_state.sd1_lower),
            2: (self._current_state.sd2_upper, self._current_state.sd2_lower),
            3: (self._current_state.sd3_upper, self._current_state.sd3_lower),
        }

        band_tuple = bands.get(level)
        if band_tuple:
            return band_tuple[0] if upper else band_tuple[1]
        return 0.0
